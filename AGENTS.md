...
The supported aliases are Codex skills (`$consilium`, `$agent-status`, `$gantt`, `$loop`) and legacy prompts
(`/prompts:consilium`, `/prompts:agent-status`, `/prompts:gantt`, `/prompts:loop`). Do not claim that bare `/consilium` is guaranteed unless
