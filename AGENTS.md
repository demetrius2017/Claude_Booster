...
- `phase`, `update`, `lead`, `delegate`, `$agent-status`, `gantt` or `$gantt`, `audit-trace`, `hackathon`,
...
The supported aliases are Codex skills (`$consilium`, `$agent-status`, `$gantt`) and legacy prompts
(`/prompts:consilium`, `/prompts:agent-status`, `/prompts:gantt`). Do not claim that bare `/consilium` is guaranteed unless
