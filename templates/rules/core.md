...
# Work Principles
- **[CRITICAL] Business outcome governs the process — proportionality precedence:** The Lead is responsible for delivering the user's verified business result within time and token constraints. Optimize delivered value per time/token, not the number of checks, agents, or reports. Start with the observable acceptance criteria and business/domain invariants; technical checks serve those criteria. Skepticism targets a concrete material failure, not an endless search for reasons to distrust a passing result.
  - Work in the largest coherent reversible slice. Reuse current evidence for unchanged scope, pass compact relevant context, and avoid repeated broad rereads or microdelegation that adds no independent evidence.
  - **Stop rule:** once the agreed acceptance checks pass and material risks are covered, deliver. Reopen verification only for a new change, a failure, or a concrete unresolved material risk; state that trigger and run the affected checks. Do not restart broad reviews merely because another review is possible.
  - **For reversible prompt/documentation edits only**, a targeted existing check (including direct hook output when applicable) plus one focused review is sufficient. This takes precedence over generic file-count escalation, paired-spawn, full-suite, and repeated audit ceremonies in `pipeline.md` and `paired-verification.md`; file count alone is not a risk. No notebook or extra agents are required solely to restate an established decision.
  - This exception does **not** cover production code/config behavior, data, auth/security, financial logic, destructive actions, incident-preservation controls, or their verification requirements. It never permits weakening those controls through prompt edits. Explicitly requested full `/go`, `/audit`, and other command contracts still apply. Quality is demonstrated by the business result and preserved invariants, not waived to save tokens.
