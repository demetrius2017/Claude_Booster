...
- **[CRITICAL] Model routing for delegated agents — Lead decides by complexity. Heavy lanes (`lead`/`coding`/`hard`) are Claude Opus 5; cheap lanes (`trivial`/`recon`/`medium`) stay on flat-fee Codex:**
  ...
  In Codex CLI, also pass the route's explicit `reasoning_effort` for delegated subagents unless the user explicitly requests higher effort; omitted effort inherits the Lead and can accidentally spawn a Terra or Sol delegate at `high`.
  ...
  - **Bio-agent** (consilium opinion agents, audit lens agents, hackathon workers) → `codex_worker.sh gpt-5.6-sol` via Bash at medium effort. Sol is the current flat-fee reasoning route for these roles. Do NOT use Agent tool for bio-agents; pipe the task to the balancer-selected route.
