...
- **[CRITICAL] /lead model override for supervised workers:** When spawning workers via `/lead`, always pass `--model` explicitly. Default: `--model claude-opus-4-8` for coding tasks. `--model claude-sonnet-4-6` only for purely mechanical work (grep-replace, formatting). Never omit `--model` — without it, the worker inherits whatever `CLAUDE_BOOSTER_MODEL` env var is set to (or no model override at all).
